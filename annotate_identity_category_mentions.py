...
class IdentityAnnotator():
    ...
    def annotate(self, descs, desc_colname, suffix, eval_mode):
        ...
        with Pool(15) as pool:
            for i,cat in enumerate(tqdm(sorted(self.terms)), ncols=50):
...
def main():
    ...
    # I/O
    descs_fpath = '/data/tumblr_community_identity/dataset114k/matched_reblogs_nonreblogs_dataset114k.csv'
    outpath = descs_fpath[:-4] + '_annotated.csv'
    resources_dirpath = '/data/tumblr_community_identity/resources'
    ...
    sys.stdout.flush()
    ia = IdentityAnnotator(resources_dirpath)
    suffixes = ['follower', 'followee_reblog', 'followee_nonreblog'] 
        # will be appended to columns; should be in the order of the desc_colnames
    for desc_colname, suffix in zip(desc_colnames, suffixes):
