...
def preprocess(desc_data, desc_colname):
    ...
    desc_data.loc[:, new_colname] = tokenize_lowercase(desc_data[new_colname])

    # # Process URLs (to hyphens) so don't interact with list descriptions
    desc_data.loc[:, new_colname] = process_urls_column(desc_data[new_colname])
