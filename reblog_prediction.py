...
from run_model import Experimenter
...
def get_args():
    ...
    parser.add_argument('--feature', dest='feature_type', help='feature type', default='user')
...
def main():
    ...
    args = get_args()
    run_output_dirpath = os.path.join(args.output_dirpath, '+'.join([args.model_name, args.feature_type, args.classifier]).strip('+'))

    # Load trained embedding models
    print("Loading embeddings...")
    emb_loader = EmbeddingLoader(args.feature_type)
    word_embs = emb_loader.word_embs

    # Load and filter dataset
    print("Loading and filtering data...")
    data_handler = DataHandler()
    data_handler.load(args.data_location, args.task)
    data_handler.filter(word_list=word_embs.keys())
    dataset = data_handler.data

    # Extract features
    print("Extracting features...")
    extractor = FeatureExtractor(args.feature_type, word_embs)
    dataset = extractor.extract(data)

    # Run model
    print("Running model...")
    experimenter = Experimenter(dataset, args.classifier_type)
    experimenter.run()

    # Print output
    print(f'\tScore: {experimenter.score: .4f}')

    # Save settings, output
    #save(data_handler, extractor, experimenter)
